...
const CheckOut = () => {
    ...
    const { data: vehicleDetail, refetch } = useQuery({
    ...
    const [qrStatusSocket, setQrStatusSocket] = useState(socket);

    const navHome = (status: any, message: any) => {
        // Alert.alert("Lấy xe", statusState.statusMessage)
        dispatch(
            generalActions.showAlert({
                isSuccess: true,
                title: 'Lấy xe',
                description: 'Chúc mừng bạn đã lấy xe thành công',
            }),
        );
                ...
                <View style={styles.qrContainer}>
                    ...
                    {vehicleDetail && (
                        <>
                            ...
                            <FieldValue
                                fieldName="Tổng thời gian"
                                value={`${String(vehicleDetail.duration.hours).padStart(2, '0')} giờ ${String(
                                    vehicleDetail.duration.minutes,
                                ).padStart(2, '0')} phút`}
                            />
